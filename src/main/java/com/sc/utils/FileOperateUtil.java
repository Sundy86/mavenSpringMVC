...
public class FileOperateUtil {
    ...
    public static List<Map<String, Object>> upload(HttpServletRequest request, String[] params, Map<String, Object[]> values) throws Exception {
        ...
        // 文件上传后存放的地址
        String uploadDir = request.getSession().getServletContext().getRealPath("/") + FileOperateUtil.UPLOADDIR+File.separator;
        ...
        for (Iterator<Map.Entry<String, MultipartFile>> it = fileMap.entrySet().iterator(); it.hasNext(); i++) {
            ...
            if(fileName!=null && !"".equals(fileName) && !"null".equals(fileName)){
               // request.setAttribute("filename",storeName+"zip");
                System.out.println(storeName+"--------------------------------------------------");
                Map<String, Object> map = new HashMap<String, Object>();
                // 固定参数值对
                map.put(FileOperateUtil.REALNAME, fileName); // 图片元名
    ...
    public static void download(HttpServletRequest request, HttpServletResponse response, String storeName,
        ...
        // 下载源路径
        String ctxPath = request.getSession().getServletContext().getRealPath("/") + FileOperateUtil.UPLOADDIR+File.separator;
